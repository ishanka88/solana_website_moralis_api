...
from website.modles import Credentials,BackupFolder
from google.cloud import bigquery
from google.oauth2 import service_account
...
def get_transactions_from_Date_and_Time(contract_address, ticker, fromDateTime, toDateTime, low_value, peak_value, discription, priority):
    """Fetches transactions from BigQuery for a given time range and other filters."""
    ...
    if not os.path.exists(key_path):
        message = f"Error - The credential file '{credential_file}' does not exist at the path '{key_path}'."
        return False, message
    ...
    try:
        ...
        message = "Error - Could not create credentials from the provided file.\n\n check the file you provided for google auth credentials"
        return False, message
    ...
    try:
        ...
        thread_data.client = client  # Store the client for later use in cancellation
        ...
        if results.total_rows > 0:
            try:
                ...
                addDetails =BackupFolder.add_record(ticker,contract_address,fromDateTime,toDateTime,from_signature,to_signature,from_block_timestamp,to_block_timestamp,low_value,peak_value,discription,priority,txns,file_name,0)
                if addDetails :
