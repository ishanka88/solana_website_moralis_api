from website.modles import AvailableCoinSets, CoinTransactions, Pairs,BackupFolder
...
def addCoinTransactions(file_all_data):
    try:
        ...
        for row in data:
            ...
            block_timestamp = 1111111   #row["block_timestamp"]   # Assuming you have the block timestamp somewhere in the data
            ...
            for account in accounts:
                if (account["signer"] == "true" and account["writable"] == "true"):
                    fee_payer = account["pubkey"]
                    break

            # Calculate pre and post token balances for the fee payer
            for balance in pre_token_balances:
                if balance["mint"] == meme_token_address and balance["owner"] == fee_payer:
                    pre_token_balance = int(balance['amount']) * Decimal(10**-int(balance['decimals']))
                    break

            for balance in post_token_balances:
                if balance["mint"] == meme_token_address and balance["owner"] == fee_payer:
                    post_token_balance = int(balance['amount']) * Decimal(10**-int(balance['decimals']))
                    break

            # Check if pre_token_balances contains only the meme token address
            x = 0
            for balance in pre_token_balances:
        ...
        if response[0]:
            ...
            coin_set_data = {
                ...
                'wallet_transfers_count': wallet_transfers_count,
